...
function AllPoliciesPage() {
  ...
  const categories = ["All", "Employment", "Service Agrmt", "Online Policies", "Consumer/Refund"];
  ...
  const sortedPolicies = useMemo(() => {
    return [...filteredPolicies].sort((a, b) => {
      const scoreA = a.overallRiskScore !== undefined ? Math.max(0, 100 - a.overallRiskScore) : (a.coverageScore || 0);
      const scoreB = b.overallRiskScore !== undefined ? Math.max(0, 100 - b.overallRiskScore) : (b.coverageScore || 0);
      switch (sortOption) {
        case "score-desc":
        ...
        case "date-desc":
          return (
            new Date(b.createdAt || b.capturedDate).getTime() -
            new Date(a.createdAt || a.capturedDate).getTime()
          );
        case "date-asc":
          return (
            new Date(a.createdAt || a.capturedDate).getTime() -
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      addToast("Agreement safely removed.", "success");
    } catch (err) {
      console.error(err);
  ...
  const handleToggleFavorite = async (e, policy) => {
    ...
    try {
      ...
      setPolicies((prev) =>
        prev.map((p) =>
          p.id === policy.id ? { ...p, isFavorite: newStatus } : p,
        ),
      );
      addToast(
            ...
            >
              <div className="all-policies__carousel">
                {categories.map((cat, idx) => {
                  ...
                  if (distance > categories.length / 2)
              ...
              <div className="all-policies__carousel-indicator">
                <div
                  className="all-policies__carousel-dot"
          ...
          <div className="all-policies__header">
            ...
            >
              <h1 className="heading-display all-policies__title">
                Your Agreement <span className="text-gradient--blue">Vault</span>
              </h1>
              <motion.p 
              ...
              >
                Manage all your analyzed contracts, online policies, and agreements.
              </motion.p>
            </motion.div>
            ...
            {policies.length > 0 && (
              <div className="all-policies__sort-pill">
                ...
                >
                  ...
                  <option value="score-desc">Protection: High to Low</option>
                  <option value="score-asc">Protection: Low to High</option>
                  ...
                  {sortOption === "date-desc"
                    ...
                    : sortOption === "date-asc"
                      ...
                      : sortOption === "score-desc"
                        ? "Protection: High to Low"
                        : sortOption === "score-asc"
                          ? "Protection: Low to High"
                          : sortOption === "risks-desc"
                            ? "Most Risks"
                            : "Fewest Risks"}
                </span>
                <svg
      ...
      <div className="container" style={{ paddingTop: "var(--space-3xl)" }}>
        {policies.length === 0 ? (
          <div className="all-policies__empty">
            ...
            <p>You haven't analyzed any agreements yet.</p>
          ...
          <div className="all-policies__empty">
            ...
            <p>No {filterType} agreements found in your vault.</p>
            ...
            {sortedPolicies.map((policy, idx) => (
              ...
              >
                <div className="all-policies__card-header">
                  {(() => {
                    const protectionScore = policy.overallRiskScore !== undefined 
                      ? Math.max(0, 100 - policy.overallRiskScore) 
                      : (policy.coverageScore || 0);
                    return (
                        ...
                        <div className="all-policies__score-ring">
                          <svg viewBox="0 0 48 48" className="all-policies__dial">
                            ...
                            <circle
                              ...
                              strokeWidth="4"
                              strokeLinecap="round"
                              strokeDasharray={`${((protectionScore || 0) / 100) * 125.6} 125.6`}
                            />
                          </svg>
                          <span className="all-policies__score-val">
                            {protectionScore}
                          </span>
                        </div>
                        <div>
                          <span className="all-policies__type">
                            {policy.policyOverview?.type || "Standard"}
                          </span>
                        </div>
                      </div>
                    );
                ...
                <div className="all-policies__info">
                  <h3 className="heading-card" style={{ marginBottom: '4px' }}>{policy.policyOverview?.name || "Unknown Agreement"}</h3>
                  <p className="subheading-muted all-policies__provider" style={{ fontSize: '0.9rem' }}>
                    {policy.policyOverview?.provider || "Unknown Party"}
                  </p>
                </div>

                <div className="all-policies__footer">
                  <span className="all-policies__date">
                    {policy.capturedDate || "Recent"}
                  </span>
                  {policy.riskFlags && policy.riskFlags.length > 0 && (
                    <span className="all-policies__risk-pill">
                      {policy.riskFlags.length} Risks Detected
                    </span>
                  )}
                </div>
              </Card>
        ...
        <ConfirmModal
          ...
          title="Permanently Delete Agreement"
          message="Are you sure you want to completely remove this agreement? This action cannot be undone and will affect your overall vault stats."
