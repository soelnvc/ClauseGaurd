...
function ProfilePage() {
  ...
  const stats = useMemo(() => {
    if (!contracts || contracts.length === 0) return { count: 0, score: '—', risks: 0 };
    
    const totalScore = contracts.reduce((acc, p) => acc + (p.riskScore || 0), 0);
    const totalRisks = contracts.reduce((acc, p) => acc + (p.exploitativeClauses?.length || 0), 0);
    const avgScore = Math.round(totalScore / contracts.length);
        ...
        <div className="container">
          ...
          <div className="profile__stats">
            <Card variant="lifted" className="profile__stat-card">
              ...
              <p className="profile__stat-label">Contracts Analyzed</p>
            </Card>
            <Card variant="lifted" className="profile__stat-card">
              <p className="profile__stat-number">{stats.score}</p>
              <p className="profile__stat-label">Avg. Risk Score</p>
            </Card>
            <Card variant="lifted" className="profile__stat-card">
                ...
                <Card variant="lifted" className="profile__actions-card">
                  ...
                  <div className="profile__action-list">
                    <Link to="/workspace" className="profile__action-item">
                      ...
                      <div>
                        <p className="profile__action-label">Analyze a Contract</p>
                    ...
                    <Link to="/compare" className="profile__action-item">
                      ...
                      <div>
                        <p className="profile__action-label">Compare Contracts</p>
                    ...
                    <div className="profile__activity-list">
                      {contracts.slice(0, 5).map((contract) => (
                        <div 
                          ...
                          onClick={() => navigate('/workspace', { state: { policy: contract } })}
                        >
                          <div className="profile__activity-icon">📄</div>
                          <div className="profile__activity-info">
                            <p className="profile__activity-name">{contract.contractOverview?.name || 'Untitled Contract'}</p>
                            <p className="profile__activity-meta">
                              {contract.contractOverview?.type || 'Standard'} · {contract.analyzedAt ? new Date(contract.analyzedAt).toLocaleDateString() : 'Recently'}
                            </p>
                          </div>
                          <div className="profile__activity-score">
                            <span className="profile__activity-score-label">Risk</span>
                            <span className={`profile__activity-score-val ${contract.riskScore > 70 ? 'text-danger' : contract.riskScore > 30 ? 'text-warning' : 'text-success'}`}>
                              {contract.riskScore}
                            </span>
                          </div>
                        </div>
